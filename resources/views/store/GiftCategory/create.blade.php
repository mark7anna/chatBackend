    ...
    <div class="modal-dialog modal-lg">
      <div class="modal-content" @if(Config::get('app.locale') == 'ar') style="direction: rtl"  @else style="direction: ltr" @endif >
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLabel">{{ __('main.new_gift_category') }}</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <div class="modal-body">
            <form  id="giftcatForm" method="POST" action="{{ route('giftCategoryStore') }}" enctype="multipart/form-data">
                @csrf
            <div class="mb-3">
        ...
        <div class="modal-footer">
          ...
          <button type="submit" class="btn btn-primary" form="giftcatForm">{{ __('main.save') }}</button>
        </div>
      </div>
    </div>
  </div>
