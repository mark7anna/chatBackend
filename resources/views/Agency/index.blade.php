<!DOCTYPE html>
<html lang="en">

@include('layouts.head' , ['pageTitle' => 'Agencies'])

<body class="g-sidenav-show  bg-gray-100">
    @include('layouts.sidebar' , ['slag' => 16 , 'subSlag' => 162])
    <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
        <!-- Navbar -->
        @include('layouts.nav' , ['pageTitle' => __('main.agencies')])
        <!-- End Navbar -->
        <div class="container-fluid py-4"  @if(Config::get('app.locale')=='ar' ) style="direction: rtl" @endif s>
            <div class="row">
                <div class="col-12">
                    @include('flash-message')
                    <div class="card mb-4">
                        <div class="card-header pb-0" style="display: flex;
                justify-content: left;
                align-items: center;">

                            <h6>{{ __('main.agencies') }}</h6>

                            <button class="btn btn-primary" @if(Config::get('app.locale')=='ar' )
                            style="margin-right: auto "
                            @else style="margin-left: auto "
                            @endif
                            id="createButton" > <i class="fa fa-plus" style="margin-right: 10px"></i>  Add New</button>

                        </div>
                        <div class="card-body px-0 pt-0 pb-2">
                            <div class="table-responsive p-0">
                                <table class="table align-items-center justify-content-center mb-0">
                                    <thead>
                                        <tr>
                                            <th
                                                class="text-uppercase text-center text-secondary text-xxs font-weight-bolder opacity-7">
                                                {{ __('main.tag') }}</th>
                                                <th
                                                class="text-uppercase text-center text-secondary text-xxs font-weight-bolder opacity-7">
                                                {{ __('main.name') }}</th>
                                            <th
                                                class="text-uppercase text-center text-secondary text-xxs font-weight-bolder opacity-7 ps-2">
                                                {{ __('main.agent') }}</th>
                                            <th
                                                class="text-uppercase text-center text-secondary text-xxs font-weight-bolder opacity-7 ps-2">
                                                {{ __('main.agency_state') }}</th>
                                            <th
                                                class="text-uppercase text-center text-secondary text-xxs font-weight-bolder opacity-7 ps-2">
                                                {{ __('main.gold_target') }}</th>
                                            <th
                                                class="text-uppercase text-center text-secondary text-xxs font-weight-bolder opacity-7 ps-2">
                                                {{ __('main.accept_hosters') }}</th>
                                            <th
                                                class="text-uppercase text-center text-secondary text-xxs font-weight-bolder opacity-7 ps-2">
                                                {{ __('main.auto_join_accept') }}</th>
                                                <th
                                                class="text-uppercase text-center text-secondary text-xxs font-weight-bolder opacity-7 ps-2">
                                                {{ __('main.auto_exit_accept') }}</th>

                                            <th></th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @foreach ($agencies as $agency )
                                        <tr>
                                            <td class="text-center"> {{ $agency -> tag }}</td>
                                            <td class="text-center"> {{ $agency -> name }}</td>
                                            <td class="text-center"> {{ $agency -> user_name }}</td>
                                            <td class="text-center">
                                                @if($agency -> active == 1)
                                                <span class="badge badge-success">{{ __('main.active') }}</span>
                                                @else
                                                <span class="badge badge-danger">{{ __('main.inActive') }}</span>
                                                @endif

                                            </td>
                                            <td class="text-center"> {{ $agency -> monthly_gold_target }}</td>
                                            <td class="text-center">
                                                @if($agency -> allow_new_joiners == 1)
                                                <span class="badge badge-success">{{ __('main.yes') }}</span>
                                                @else
                                                <span class="badge badge-danger">{{ __('main.no') }}</span>
                                                @endif

                                            </td>
                                            <td class="text-center">
                                                @if($agency -> automatic_accept_joiners == 1)
                                                <span class="badge badge-success">{{ __('main.yes') }}</span>
                                                @else
                                                <span class="badge badge-danger">{{ __('main.no') }}</span>
                                                @endif

                                            </td>
                                            <td class="text-center">
                                                @if($agency -> automatic_accept_exit == 1)
                                                <span class="badge badge-success">{{ __('main.yes') }}</span>
                                                @else
                                                <span class="badge badge-danger">{{ __('main.no') }}</span>
                                                @endif

                                            </td>
                                            <td class="text-center">
                                                <button type="button" class="btn btn-success editBtn" value="{{ $agency -> id }}"><i class="fas fa-edit"></i></button>
                                              <button type="button" class="btn btn-danger deleteBtn" value="{{ $agency -> id }}"><i class="far fa-trash-alt"></i></button>
                                              </td>


                                        </tr>
                                        @endforeach

                                    </tbody>
                                </table>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            @include('layouts.fixedPlugin')

            @include('layouts.footer')
            @include('Agency.create')
            @include('Agency.deleteModal')

            <script type="text/javascript">
                $(document).on('click', '#createButton', function (event) {

              event.preventDefault();
              let href = $(this).attr('data-attr');
              $.ajax({
                  type:'get',
                  url:'/getAgencyTag',
                  dataType: 'json',

                  success:function(response){
                      console.log(response);
                      if(response){
                          let href = $(this).attr('data-attr');
                          $.ajax({
                  url: href,
                  beforeSend: function () {
                      $('#loader').show();
                  },
                  // return the result
                  success: function (result) {
                      $('#createModal').modal("show");
                      $(".modal-body #name").val("");
                      $(".modal-body #tag").val(response);
                      $(".modal-body #user_id").val("");
                      $(".modal-body #monthly_gold_target").val("");
                      $(".modal-body #details").val("");
                      $(".modal-body #active").prop( "checked", false );
                      $(".modal-body #allow_new_joiners").prop( "checked", false );
                      $(".modal-body #automatic_accept_joiners").prop( "checked", false );
                      $(".modal-body #automatic_accept_exit").prop( "checked", false );
                      $(".modal-body #id").val(0);
                      $(".modal-body #profile-img-tag").attr('src', '{{ asset('assets/icons/picture.png') }}');

                  },
                  complete: function () {
                      $('#loader').hide();
                  },
                  error: function (jqXHR, testStatus, error) {
                      console.log(error);
                      alert("Page " + href + " cannot open. Error:" + error);
                      $('#loader').hide();
                  },
                  timeout: 8000
              })
                      } else {

                      }
                  }
              });


          });

          $(document).on('click', '.editBtn', function(event) {
              let id = event.currentTarget.value ;
              console.log(id);
              event.preventDefault();
              let href = $(this).attr('data-attr');
              $.ajax({
                  type:'get',
                  url:'/getAgency' + '/' + id,
                  dataType: 'json',

                  success:function(response){
                      console.log(response);
                      if(response){
                          let href = $(this).attr('data-attr');
                          $.ajax({
                              url: href,
                              beforeSend: function() {
                                  $('#loader').show();
                              },
                              // return the result
                              success: function(result) {
                                  $('#createModal').modal("show");

                                  // var img =  response.icon ;
                                   var img =  '/../images/Badges/' + response.icon ;
                                    $(".modal-body #profile-img-tag").attr('src' , img );
                                    $(".modal-body #name").val(response.name);
                                    $(".modal-body #tag").val(response.tag);
                                    $(".modal-body #tag").val(response.tag);
                                    $(".modal-body #monthly_gold_target").val(response.monthly_gold_target);
                                    $(".modal-body #details").val(response.details);
                                    $(".modal-body #active").prop( "checked", response.active == 1 );
                                    $(".modal-body #allow_new_joiners").prop( "checked", response.allow_new_joiners == 1 );
                                    $(".modal-body #automatic_accept_joiners").prop( "checked", response.automatic_accept_joiners == 1 );
                                    $(".modal-body #automatic_accept_exit").prop( "checked", response.automatic_accept_exit == 1 );
                                    $(".modal-body #id").val(response.id);

                              },
                              complete: function() {
                                  $('#loader').hide();
                              },
                              error: function(jqXHR, testStatus, error) {
                                  console.log(error);
                                  alert("Page " + href + " cannot open. Error:" + error);
                                  $('#loader').hide();
                              },
                              timeout: 8000
                          })
                      } else {

                      }
                  }
              });
          });
          $(document).on('click', '.deleteBtn', function(event) {
              id = event.currentTarget.value ;
              console.log(id);
              event.preventDefault();
              let href = $(this).attr('data-attr');
              $.ajax({
                  url: href,
                  beforeSend: function() {
                      $('#loader').show();
                  },
                  // return the result
                  success: function(result) {
                      $('#deleteModal').modal("show");
                  },
                  complete: function() {
                      $('#loader').hide();
                  },
                  error: function(jqXHR, testStatus, error) {
                      console.log(error);
                      alert("Page " + href + " cannot open. Error:" + error);
                      $('#loader').hide();
                  },
                  timeout: 8000
              })
          });
          $(document).on('click', '.btnConfirmDelete', function(event) {

              confirmDelete(id);
          });
          $(document).on('click', '.cancel-modal', function(event) {
              $('#deleteModal').modal("hide");
              console.log()
              id = 0 ;
          });

          function confirmDelete(id){
              let url = "{{ route('deleteAgency', ':id') }}";
              url = url.replace(':id', id);
              document.location.href=url;
          }

            </script>
        </div>
    </main>

    <!--   Core JS Files   -->


</body>

</html>
